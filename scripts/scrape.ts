...
import { ADAPTERS, adapterFor, collect, keyOf, mergePosts } from "@/lib/scrape";
...
const VALUE_FLAGS = new Set(["pages", "delay"]);
...
async function main() {
  ...
  console.log(`저장: ${path.relative(process.cwd(), OUT_FILE)} — 전체 ${merged.length}건 (신규 ${added}건)`);
