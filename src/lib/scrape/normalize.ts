import { POSITIONS, type Employment, type Position, type Region } from "@/lib/jobs";
import { parsePlace } from "@/lib/region";

/**
 * 지역 표기를 시·도로 읽는다.
 *
 * 나누는 규칙은 src/lib/region.ts 한 곳에만 둔다. 수집기와 화면이 서로 다른
 * 규칙을 쓰면, 같은 공고가 수집할 때와 볼 때 다른 지역에 들어간다.
 */
export function normalizeRegion(raw: string | null | undefined): Region | null {
  return parsePlace(raw).sido;
}
