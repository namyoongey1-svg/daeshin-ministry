...
export default function LoginPage() {
  ...
  return (
    <div className="mx-auto max-w-sm">
      ...
      {stage === "form" ? (
        ...
        <>
          <div className="mt-6 rounded-lg border border-line bg-surface p-4 text-sm leading-relaxed">
            <b>{email}</b> 으로 메일을 보냈습니다. 메일 안의 링크를 누르세요.
            <button
          ...
          {isLocal && (
            <p className="mt-3 rounded border border-highlight px-3 py-2 text-xs leading-relaxed">
              <b>지금은 이 컴퓨터에서만 열립니다.</b> 링크가 <code>localhost</code>로
              돌아오기 때문에, 휴대폰이나 카카오톡·메일 앱 안에서 누르면 그 기기의
              localhost를 찾다가 실패합니다. 이 컴퓨터의 브라우저에서 메일을 열어
              주세요. 사이트를 배포하면 어느 기기에서든 열립니다.
            </p>
          )}
      ...
      {error && (
        <p className="mt-4 rounded border border-highlight px-3 py-2 text-xs leading-relaxed text-highlight">
          {error}
        </p>
      )}
    </div>
